...
| Trans: "cs'' \<leadsto>*[\<tau>] cs' \<Longrightarrow> cs \<leadsto>[\<sigma>] cs'' \<Longrightarrow> cs \<leadsto>*[(\<tau> \<circ>m \<sigma>)] cs'"
...
definition red_cs :: "constraint_system \<Rightarrow> msg_subst set" where
  "red_cs cs = {\<tau> \<circ>m \<sigma> | \<tau> \<sigma>. (\<exists>cs'. cs \<leadsto>*[\<sigma>] cs' \<and> is_simple_cs cs' \<and> \<tau> \<in> sol(cs'))}"
...
proof(cases rule: rer1.cases)
  ...
  then have "\<sigma> \<cdot>m t = \<sigma> \<cdot>m u"
    ...
    unfolding msg_is_mgu_alt msg_unifiess_alt msg_unifies_alt
    by force
...
theorem red_soundness: "red_cs cs \<subseteq> sol(cs)"
  using sol_def red_cs_def lemma_9 
  by force
